...
| `tsconfig.base.json` | Shared compilerOptions and the source `paths` map. Also the resolution facade used by Vitest through Vite's native `resolve.tsconfigPaths`: it has no `include`, so its `paths` apply to every importer. | No |
