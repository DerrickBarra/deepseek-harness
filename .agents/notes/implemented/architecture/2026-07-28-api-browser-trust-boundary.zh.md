...
已信任的 authority 获得与回环等价的 DSH API 访问。这包括 settings、credentials、宿主桌面操作、带调用方端点数据的宿主侧模型发现，以及 agent preset 创作。这个等价性是可信部署的明确选择；认证层仍是延期工作。
...
- **让一部分宿主 API 保持回环专属。** 否决：部署已经点名其服务 authority 后，这会让受信任的远程操作者只得到不完整的宿主 API。Settings、credentials、模型发现、宿主操作与 preset 创作需要共用同一个可达性决定，使受信任的远程会话表现得像回环。
