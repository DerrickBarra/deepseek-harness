...
已信任的 authority 可以访问 settings 面（`settings.describe`、`settings.openDocument`、`settings.update`、`settings.replace` 与 `settings.mutate`），因此远程配置界面与回环浏览器一样使用宿主持久化的 settings 状态。回环专属集合仍保留本机桌面操作、secret store 读写、带调用方端点数据的宿主侧模型发现，以及 agent preset 创作。
...
- **让所有 settings RPC 保持回环专属。** 否决：即使部署已经点名其服务 authority，受信任的远程操作者仍无法打开或持久化普通配置界面。
...
- Settings UI 状态会为受信任的远程浏览器持久化，而不是退回到进程本地内存。
