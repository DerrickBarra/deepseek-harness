...
**欢迎声明复用既有持久化字段。** 完整文案与版本由 `onboarding-copy.ts` 持有。通过 `/api` 信任栅栏的客户端经既有 settings API 比较和写入 `ui-onboarding.welcomeNoticeVersion`，且只有点击「继续」才确认当前版本。被拒绝的客户端继续使用既有的进程内回退。不改变 Host schema、API Proxy 允许列表或持久化实现。
