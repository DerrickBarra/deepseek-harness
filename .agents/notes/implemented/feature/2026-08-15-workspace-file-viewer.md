...
The Web bundle now mounts an additional Host Remote and client UI plugin by default. The default root is intentionally deployment-specific to the OpenClaw Chip spike; deployments that do not want that path can override or replace the `workspace-file-viewer` row in `cordis.patch.yml`.
