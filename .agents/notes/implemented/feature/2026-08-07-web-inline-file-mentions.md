...
**The renderer owns no vocabulary, and deliverables contributes the stock provider.** `MarkdownText` takes an optional `MarkdownFileMentions` resolver and consults it for inline-code tokens — after URL promotion, which wins, and never inside an anchor, where a button cannot nest. What names a file is decided behind ui-chat's additive `chatFileMentions` registry ([provider-registry decision](../architecture/2026-09-05-chat-file-mention-provider-registry.md)): ui-deliverables registers its priority-0 provider beside its turn-tail chain entry, so one cordis.yml line composes the row and its prose links in or out together, while other providers can coexist and ui-primitives gains no session concepts. Mentions apply to settled renders only — the streaming cache must not bake in handlers that could go stale, and the vocabulary is not final until the turn closes. The consumer memoizes each resolver on closing-message identity and the live provider roster, so a settled message's cached parse survives stream appends but refreshes when providers change.
...
- **Threading the vocabulary through the turn-tail chain** — the chain is a render dispatch below the message; mentions decorate markdown inside it, which only data reaching MarkdownText can do. The Chat-owned provider registry is that data path, and an empty roster is the off state.
