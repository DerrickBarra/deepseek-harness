...
通过配置创建的 agent 会自动启动。模型调用同时需要 `provider` 和 `model`；`agent/request` 可以在分发前补齐缺失的这一对值。可选的正数 `maxTokens` 会为每次对话请求提供初始输出上限，并记录在请求 header 中。`maxParallelToolCalls` 限制每个 agent 针对并行安全调用使用的滚动池，默认值为 `10`。`reasoningOnlyStopRetries` 默认值为 `2`，控制正常停止只包含推理而没有最终文本或工具调用时的重试次数；`0` 会在首次出现时报告。两个字段都属于 `agent-loop` Settings 段，无需重启即可生效；无效值会在写入时被拒绝。`agents` 刻意不在该段中——它在服务启动时被消费一次，所以存储的改动只会看起来生效。`cwd` 仅应用于全新会话，而 `resumeSessionId` 保留持久化元数据。通过配置创建的 agent 使用部署 persona；编程式 setup 可以按 agent 遮蔽它。该插件为每个 agent 提供 `provider`、`model` 和 `cwd` 提示词变量；harness 身份与部署 persona 属于 `dsh-system-prompt`。
...
每次提供方调用以可用结果成功结束时，都会恰好追加一个 `assistant/message` 完成锚点，包括无内容调用和以 `max-tokens` 结束的调用。正常停止若只包含推理而没有最终文本或工具调用，则改为追加带有有界推理后缀的 `step/empty-answer`，并在同一个请求上最多重试 `reasoningOnlyStopRetries` 次；失败尝试不会进入派生历史，重试耗尽后轮次以 `EMPTY_ANSWER` 结束。该锚点原样记录组装后的内容，在 `sourceEventSeqs` 中列出确切的分片 seq（流没有分片时为 `[]`），并在用量可用时包含用量；空内容不会进入派生消息历史。轮次取消打断流式输出时，如果非空文本或推理内容已送达用户，循环也会追加一个带 `interrupted: true` 的锚点。该锚点引用对应的分片 seq，并把已渲染的前缀放入派生消息历史，使下一次请求包含用户看到的内容。未分派的工具调用会被省略，空流或只包含工具调用的流不会生成锚点；提供方故障也不提交 assistant 内容（[仅推理停止重试](../../../.agents/notes/implemented/bug-fix/2026-09-24-reasoning-only-stop-retry.zh.md)与[取消流终结](../../../.agents/notes/implemented/architecture/2026-08-10-cancelled-stream-prefix-finalize.zh.md)）。
