...
`frequencyPenalty` 接受 OpenAI 的 -2 到 2 范围，并通过 pi-ai 的 payload 钩子以 `frequency_penalty` 发送。只有路由上的每个模型都使用 `openai-completions` 时它才有效；其他协议会让 profile 解析失败，而不会静默忽略它。
