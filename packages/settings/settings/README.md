...
- `register(ns, schema, { base?, applies?, exposure?, validate? })` — returns the owner `SettingsScope` (`get`/`watch`/`update`). `exposure: 'web'` authorizes the trusted Web configuration client to describe and mutate that namespace; omission is default-deny, while the wire adapter still owns redaction, transport trust, and provider write policy. The registration is an effect on the calling plugin's fiber: disposing that fiber removes the namespace and its observers. A stored section the schema or owner validator rejects fails the registration itself; a duplicate namespace fails loud.
