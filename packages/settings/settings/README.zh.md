...
- `register(ns, schema, { base?, applies?, exposure?, validate? })` — 返回 owner 的 `SettingsScope`（`get`/`watch`/`update`）。`exposure: 'web'` 授权受信任的 Web 配置客户端描述并修改该 namespace；省略时默认拒绝，而 wire 适配器仍负责脱敏、传输信任与提供方写入策略。注册是调用方插件 fiber 上的 effect：dispose（资源释放）该 fiber 即移除 namespace 及其观察者。schema 或 owner 校验器拒绝的存量分节会使注册本身失败；重复 namespace 立即报错。
