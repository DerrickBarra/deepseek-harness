...
`agentPreset.read`, `copy`, `openDocument`, and `remove` are privileged and pass through the shared `/api` trust fence ([`dsh-client-connection`](../connection/README.md)): loopback and declared `trustedHosts` can call them, while undeclared non-loopback authorities are rejected before RPC dispatch. A composition names the plugins a session runs, so reading one is reconnaissance, and the rest manage the roster and drive the host desktop. `agentPreset.list` carries ids, trust, and the two path-free capability flags every preset picker needs.
