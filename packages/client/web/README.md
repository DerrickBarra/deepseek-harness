...
The shell owns browser-title projection over the branding service. With a selected session carrying a durable title, it renders `<session title> — <branding productTitle>` and reacts to session or branding revisions; no selection or a selected untitled session renders the active product title, and shell unmount restores the pre-shell HTML title.
