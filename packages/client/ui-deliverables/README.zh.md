...
与 `ui-conversation` 和 `ui-chat` 一起挂载本插件。`ui-chat` 是必需依赖，因为它拥有本包安装 stock provider 所需的 `chatFileMentions` registry；缺少该 service 时，ui-deliverables Client fiber 会保持 pending，而不会部分挂载。已完成轮次随即以产出文件行收尾，位于收尾消息正文与其动作页脚之间。每个标签项经 Host 打开器打开文件，相对路径按会话 cwd 解析；该行首次显示时会查询 `session.canOpenWorkspacePath()`，有文件被省略、页面为 loopback 且查询成功返回 `true` 时，**在文件夹中显示**动作才会打开会话工作区。
