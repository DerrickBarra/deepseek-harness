...
声明步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。通过 `/api` 信任栅栏的浏览器会经既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确点击「继续」才会记录当前版本。被拒绝的浏览器无法使用这项 Host namespace，因此确认仅在当前进程有效，重载后声明会再次出现。
